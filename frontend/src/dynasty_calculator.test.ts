...
describe("buildQuickStartSettings", () => {
  it("builds points quick start with points defaults and normalized common settings", () => {
    ...
    expect("enable_bench_stash_relief" in settings).toBe(false);
    expect("bench_negative_penalty" in settings).toBe(false);
    expect("enable_ir_stash_relief" in settings).toBe(false);
    expect("ir_negative_penalty" in settings).toBe(false);
  ...
  it("builds deep dynasty quick start with exact league settings and no exposed realism flags", () => {
    const settings = buildQuickStartSettings({
      mode: "deep",
      settings: baseSettings,
      availableYears: [2026, 2027],
